...
public class StableVersionsStrategy implements VersionsStrategy{
	...
	public void putVersion(Document document) {
		String filename = "./versions/" + document.getVersionID() + ".tex";
	...
	public Document getVersion() {
		...
		String fileContents = "";
		try {
			Scanner scanner = new Scanner(new FileInputStream("./versions/" + versionID + ".tex"));
	...
	public void setEntireHistory(List<Document> documents) {
		for(int i = 0; i < documents.size(); i++) {
			...
			doc.save("./versions/" + doc.getVersionID() +".tex");
		}
		if(documents.size() > 0)
	...
	public List<Document> getEntireHistory() {
		...
		for(int i = 0; i <= n; i++) {
			String fileContents = "";
			try {
				Scanner scanner = new Scanner(new FileInputStream("./versions/" + i + ".tex"));
				while(scanner.hasNextLine()) {
					fileContents = fileContents + scanner.nextLine() + "\n";
				}
			} catch (FileNotFoundException e) {
